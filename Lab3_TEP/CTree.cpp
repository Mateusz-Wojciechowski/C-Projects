...
CNode* CTree::vCreateTree(string s_prefix, int &i_index){
    ...
    if (bIsOperator(s_expression)) {
        ...
        for (int i = 0; i < operandsNeeded; i++) {
            ...
            if (s_curr_op == S_HASH) {
                if (i == 0 && !bIsNum(c_child->getValue())) {
                    b_wrong_arguments = true;
                } else if (i == 1 && !bIsVariable(c_child->getValue())) {
                    b_wrong_arguments = true;
                }
            }
        }
...
int CTree::getOperandsNeeded(const string op) {
    if(op==S_PLUS || op==S_SUB || op==S_MUL || op==S_DIV || op==S_HASH){
...
void CTree::vCreateMap(){
    ...
    operations.insert(make_pair(string(S_HASH), COperation(COperation::i_hash)));
}
